...
(function (window, factory) {
  ...
  TMap.prototype = {
    ...
    //改变overlay属性
    changeAttribute: function (options) {
      var config = {
        type: null,
        icon: null,
        position: null,
      ...
      mapPromise.promise.then(function () {
        ...
        if (config.setViewport) {
          if (!config.viewPortList) {
            return
          }
          var pointList = [];
          for (var i = 0; i < config.viewPortList.length; i++) {
            pointList.push(new BMap.Point(config.viewPortList[i].lng, config.viewPortList[i].lat))
          }
          _this.map.setViewport(pointList, {
            margins: config.viewPortMargins,
            zoomFactor: config.viewPortZoomFactor
          });
        }
        if (config.animation) {
          if (config.animation == 1) {
            config.overlay.setAnimation(BMAP_ANIMATION_DROP);
          } else {
            config.overlay.setAnimation(BMAP_ANIMATION_BOUNCE);
          }
        }
        if (config.center) {
    ...
    pullBox: function (options) { //拉框搜索
      ...
      mapPromise.promise.then(function () {
        if (!config.name) {
          ...
          var pullBox = config.name;
          pullBox.setKeyword(config.keyword);
        }
        pullBox.open();
        if (config.callback) {
